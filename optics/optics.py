...
from utils import dist1, dist2, DBSCAN_COLOR_DICT, annotate_points
import os
...
def point_plot(X: np.ndarray, X_dict: Dict[str, np.ndarray], o: str,
               eps: float, processed: Optional[Iterable] = None, col: str = "yellow", name: str = None) -> None:
    """
    ...
    plt.savefig(f"images/point_plot_{name}.png")
...
def reach_plot(data: Dict[str, np.ndarray], ClustDist: Dict[str, float], eps: float) -> None:
    ...
    plt.savefig("images/reach_plot{}.png".format(len(ClustDist)))
...
def OPTICS(X: np.ndarray, eps: float, minPTS: int, plot: bool = True,
    ...
    while len(processed) != len(X):
        ...
        if plot is True:
            ...
            point_plot(X, X_dict, o, eps, processed, name=str(len(processed)))
